...
(*       ignore(L.build_store lregexp left_tree_ptr b);
    ...
    let rec stmt builder callback_list = function
	      SBlock sl -> List.fold_left stmt builder callback_list sl
      ...
      | SIf (predicate, then_stmt, else_stmt) ->
         ...
         in let then_builder = stmt (L.builder_at_end context then_bb) callback_list then_stmt
         ...
         in let else_builder = stmt (L.builder_at_end context else_bb) callback_list else_stmt
      ...
      | SWhile (predicate, body) ->
          ...
          in let while_builder = stmt (L.builder_at_end context body_bb) callback_list body
          ...
          (* Hook everything up *)
	        ...
	        in L.builder_at_end context merge_bb
      | SInfloop (body) -> stmt builder callback_list ( SBlock [SWhile ((A.TBool ,SBoolLit(true)), SBlock [body]) ] )
      (* Implement for loops as while loops! *)
      | SFor (e1, e2, e3, body) -> stmt builder callback_list ( SBlock [SExpr e1 ; SWhile (e2, SBlock [body ; SExpr e3]) ] )
